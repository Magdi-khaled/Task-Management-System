...
<template>
    <div class="overflow-auto min-h-[26em]">
            ...
            <tbody class="text-center relative text-primary">
                <tr v-for="(item, index) in paginatedItems" :key="index" class="h-20 odd:bg-white even:bg-gray-100">
                    ...
                    <td class="py-2 px-4 capitalize"
                        :class="{ 'text-green-600': item.status, 'text-red-400': !item.status }">
                    ...
                    <td class="py-2 px-4 relative">
                        <div v-if="!item.status">
                            ...
                            <div class="bg-white" v-if="showInfo === index">
                                <Options :show="showInfo >= 0" :task="item" :lastRow="islastRow()" />
                            </div>
                        </div>
                        <div v-else>
    ...
    <Pagination :show="tasks.length > pageSize" :currentPage="currentPage" :totalPages="totalPages" :nextPage="nextPage"
        :prevPage="prevPage" />
</template>